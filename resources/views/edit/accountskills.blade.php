...
@section('content')
    ...
    if((isset($_GET['softskills']) && !empty($_GET['softskills'])) || (isset($_GET['hardskills']) && !empty($_GET['hardskills'])))
    {
        ...
        $sql = "DELETE FROM link_requirements_users WHERE user_id=". Auth::id();
        ...
        if(isset($_GET['softskills']) && !empty($_GET['softskills']))
        {
            ...
            foreach ($softskills as $skill)
            {
                ...
                $sql = "INSERT INTO link_requirements_users (requirement_id, user_id) VALUES ($requirement_id, ".Auth::id().")";
                if ($conn->query($sql) === TRUE) {
                } else {
                    echo "Error: " . $sql . "<br>" . $conn->error;
                }
